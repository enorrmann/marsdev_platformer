...
void playerInit()
{
	...
	// used for the camera in camera.c
	...
	// Initialize double jump ability
	canDoubleJump = TRUE;
}
	...
	if (joy == JOY_1)
	{
		...
		if (changed & (BUTTON_B | BUTTON_C))
		{
			...
			if (state & ( BUTTON_B | BUTTON_C))
			{
				...
				else if (playerBody.onGround || currentCoyoteTime > 0)
				{
					currentJumpBufferTime = jumpBufferTime;
				}
				else if (!playerBody.onGround && canDoubleJump)
				{
					// Perform double jump
...
void checkCollisions()
{
	...
	if (levelLimits.max.y <= playerBounds.max.y)
	{
		...
		else
		{
			...
			// Reset double jump ability when touching the ground
			canDoubleJump = TRUE;
		}
	}
	else
	...
	playerBody.globalAABB = playerBounds; // para no calcularlo nuevamente en interactions.c
}
